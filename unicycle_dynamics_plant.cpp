void dynamics_forplant( double *x, double *f,  void  *user_data ){
    ...
    f[0] =  v*cos(psi) + dis_dynamics[0];       //dot(px)
    f[1] =  v*sin(psi) + dis_dynamics[1];       //dot(py)
    f[2] =  a + dis_dynamics[2];              //dot(v)
    f[3] =  psi_dot + dis_dynamics[3];                    //dot(psi)
   // f[4] =  10*delta_x+  0*a*a + 0*psi_dot*psi_dot;       //dot(L)
