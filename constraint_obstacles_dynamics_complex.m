...
A_n_side_pos = [-L_g_h_sid_pos,  -5*h_sid_pos/ck];
b_n_side_pos = L_f_h_sid_pos;
A_n_side_neg=  [-L_g_h_sid_neg,  -5*h_sid_neg/ck];
b_n_side_neg = L_f_h_sid_neg;
...
%     Ds = 1.2;  %the radius of obstacle 
 ...
 %% August 3th, angle constraint for obstacles, 
    ...
    if (cos_rel_ang>=-0.99) 
        ...
        A_n_angle_fix = [-L_g_h_ang, 0, -5*h_ang^3/ck];
        b_n_angle_fix = L_f_h_ang + L_t_h_ang;
...
%         alert = 1; 
    ...
    A_n_vel = [-L_g_h_vel, -20*h_vel/ck];
    b_n_vel = L_f_h_vel + L_t_h_vel;
    
 %%  August 10th, constraint 2, velocity constraint, 2nd type, only constraint the acc, test:  
    %notice there may be virtual number, nan, of inf, you should aoid these
    %conditions: 
%     Ds = 1;    
%     Ds = radius_array(i_ob)+0.0;
